...
EFI_STATUS PCIBaseEntryPoint(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
	UINT32 bAddr=0xE0000,eAddr=0xFFFFF;
	UINT32 pLow,pHigh,RSDP=0,i;
	UINT32 XsdtAddress=0,XsdtLen,EntryCount,EntryAddr;
	UINT32 MCFG=0,PCIBase=0;
	InitializeShellApplication (ImageHandle, SystemTable);
	while(bAddr < eAddr)
	{
		pLow=bAddr;
	...
	EntryCount=XsdtLen-36;
	for(i=0;i<EntryCount/8;i++)
	{
		EntryAddr=MemRead32(XsdtAddress + 36 + 8*i);
		if(MemRead32(EntryAddr) == 0x4746434D)
		{
			MCFG=EntryAddr;
			break;
		}
	...
	PCIBase=MemRead32(MCFG + 0x2C);
	Print(L"PCI Base Address is %08llx\n",PCIBase);
	return EFI_SUCCESS; 
}
